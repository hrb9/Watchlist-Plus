...
def request_media_from_overseer(imdb_id, media_type="movie"):
    ...
    try:
        ...
        if not title or not overseerr_id:
            logging.warning(f"Could not find title or Overseerr ID for IMDb ID: {imdb_id}, trying alternative media type")
            # Try the opposite media type as a fallback
            alt_media_type = "tv" if media_type == "movie" else "movie"
            r = requests.post(f"{getimdbid_url}/convert_ids", 
                            json={"imdb_id": imdb_id, "media_type": alt_media_type})
            r.raise_for_status()
            media_details = r.json()
            title = media_details.get("title")
            tvdbId = media_details.get("tvdb_id", 0)
            ...
            if not title or not overseerr_id:
                error_msg = f"Could not find title or Overseerr ID for IMDb ID: {imdb_id} with either media type"
                logging.error(error_msg)
                return {"error": error_msg}
                
            # Update media_type to the one that worked
            media_type = alt_media_type
            logging.info(f"Successfully found title with alternative media type: {alt_media_type}")
